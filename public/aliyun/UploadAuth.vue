  ...
  <div class="container">
    ...
    <div class="upload">
      <div>
        ...
        <br/><label class="status">上传状态: <span>{{statusText}}</span></label>
      </div>
...
<script>
  ...
  import { getVideoVoucher , RefreshUpload } from '../../../apis/course'
        ...
        let uploader = new AliyunUpload.Vod({
          ...
          onUploadstarted: function (uploadInfo) {
            ...
            if (!uploadInfo.videoId) {
              ...
              //axios.get(createUrl)
              getVideoVoucher(self.file.name.split('.')[0],self.file.name)
              .then(res => {
                ...
                // console.log(res.data, uploadAuth,uploadAddress,videoId)
                uploader.setUploadAuthAndAddress(uploadInfo, uploadAuth, uploadAddress,videoId)  
                self.$emit('getVideoId',{videoId:this.videoId,title:self.file.name})
              })
              ...
              RefreshUpload({videoId:uploadInfo.videoId})
              .then(res => {
                console.log( res , "我是第二次上传" )
                let uploadAuth = res.data.uploadAuth
                let uploadAddress = res.data.uploadAddress
                let videoId = res.data.videoId
                uploader.setUploadAuthAndAddress(uploadInfo, uploadAuth, uploadAddress,videoId)
